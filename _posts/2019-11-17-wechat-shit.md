...
微信维护了一份 URL 的黑名单（疑似也有 IP 地址的黑名单），当打开黑名单中的 URL 时，微信会提示用户“为了账号安全，不要再网络上输入微信用户密码”之类的提示消息。只有把网址主动输入到浏览器中才能浏览这个网址。我的 blog 不幸也惨遭毒手（可能 GitHub Pages 的 IP 地址都被拉了黑名单）。当然黑名单里面的提示也不只有这种，还有“根据用户举报，此网站包含违法内容”（值得一提的是，https://github.com/996icu/996icu 就是使用这个理由屏蔽的，由此可见这个理由并没有什么可信度）或者 “lewd content” 之类的。当然有的时候并不需要什么理由，例如屏蔽友商（淘宝，拼多多，抖音等）的时候。总而言之，在微信的地方微信具有绝对的支配权，这个屏蔽系统既不透明，也没有申诉机制，很难让人相信这只是为了所谓的”保护用户“。
