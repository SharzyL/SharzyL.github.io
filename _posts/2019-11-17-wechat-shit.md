...
更体会到了微信作为一个滥用自己的霸权地位，以破坏互联网生态为代价获取盈利的。为了发泄这些日子对于微信的不满，特此写一篇文章来记录微信给我带来了那些不满。（顺带吹一下 Telegram）

# Be evil

不难发现，微信在功能上远远落后于其它 IM。粗略来说，除了微信支付以外，微信的所有功能都落后于 QQ；除了微信支付，视频聊天以外，微信的所有功能都远落后于 Telegram。由于笔者不常用朋友圈，所以本文中暂不考虑相关内容。
...
这里聊天功能只包含是指在聊天界面（包括私聊，群聊）所体现的功能，不包括消息同步等功能（这些功能过于严重，需要单独批判）。在这一方面，各个软件事实上都有很大的差异。
...
| 消息浏览          | 可以跳到上一次之后第一条发送的消息，在有新消息时出现跳转到最新消息的按钮 | 同微信                                                       | 可以跳转到第一条**未读**的消息，在查看某个回复对应的原消息时，可以快速回到回复的位置。有常显示的回到最下按钮 |
